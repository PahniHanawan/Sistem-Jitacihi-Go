        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 bg-gradient-to-r from-amber-50 to-white border-b border-slate-100 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-500">Perbarui master data produk Anda</p>
                </div>
            </div>
            ...
            <form action="{{ route('products.update', $product->product_id) }}" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="product_code" value="{{ old('product_code', $product->product_code) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm transition-all font-mono @error('product_code') border-rose-500 @enderror">
                    @error('product_code')
                ...
                <div>
                    ...
                    <input type="text" name="product_name" value="{{ old('product_name', $product->product_name) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm transition-all @error('product_name') border-rose-500 @enderror">
                    @error('product_name')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category', $product->category) }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm transition-all @error('category') border-rose-500 @enderror">
                    @error('category')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">Status Produk</label>
                    ...
                    @error('status')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-slate-500 mt-1">Ubah ke "Non-Aktif" jika barang ini sudah tidak dijual lagi agar tidak muncul di form penilaian.</p>
                </div>

                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                    <a href="{{ route('products.index') }}" class="px-5 py-2.5 border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors text-sm">
                        Batal
                    </a>
